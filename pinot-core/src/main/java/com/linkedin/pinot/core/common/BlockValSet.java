...
public interface BlockValSet {
   ...
   *
   * @param <T> Return type
   * @return Values for multi-valuted column.
   */
  <T> T getMultiValues();
  ...
  /**
   * Returns the dictionary id for the specified doc id, for the (single-valued) column of this block.
   *
   * @param docId Doc Id for which to return the dictionary id
   * @return Dictionary id for the specified doc id.
   */
  int getDictionaryIdForDocId(int docId);
