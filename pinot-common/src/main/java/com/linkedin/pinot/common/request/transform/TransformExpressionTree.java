...
public class TransformExpressionTree {
   ...
   */
  public TransformExpressionTree(int id, String column, String literal, String transformName,
      List<TransformExpressionTree> children) {
    super();
    _id = id;
    _column = column;
    _literal = literal;
    _transform = TransformFunctionFactory.get(transformName);
    _children = children;
   ...
   */
  public static TransformExpressionTree buildTree(AstNode rootNode, int id) {
    TransformExpressionTree expressionTree;

    if (!rootNode.hasChildren()) {
      if (rootNode instanceof IdentifierAstNode) {
        return new TransformExpressionTree(id, ((IdentifierAstNode) rootNode).getName(), null, null, null);
      } else if (rootNode instanceof LiteralAstNode) {
        return new TransformExpressionTree(id, null, ((LiteralAstNode) rootNode).getValueAsString(), null, null);
      } else {
      ...
      expressionTree =
          new TransformExpressionTree(id, null, null, ((FunctionCallAstNode) rootNode).getName(), children);
      for (AstNode child : rootNode.getChildren()) {
        children.add(buildTree(child, ++id));
      }
    } else {
      throw new Pql2CompilationException(
